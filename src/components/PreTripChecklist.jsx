...
import { CheckCircle2, Circle, Plus, Trash2, AlertCircle, Download, Share2 } from 'lucide-react';
...
const PreTripChecklist = () => {
    ...
    return (
        <div className="min-h-screen pt-20 pb-20 bg-gradient-to-br from-orange-50 via-red-50 to-yellow-50">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-amber-700 text-lg">Never forget anything on your travels again</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-amber-200/50 sticky top-24">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-xl font-bold text-amber-900">My Trips</h2>
                                <button
                                    onClick={() => setShowForm(!showForm)}
                                    className="p-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-lg hover:shadow-lg transition-all"
                                    title="Add new trip"
                                >
                                    <Plus size={20} />
                                </button>
                            ...
                            {showForm && (
                                <form onSubmit={createNewChecklist} className="mb-6 pb-6 border-b border-amber-200">
                                    <input
                                        type="text"
                                        placeholder="Trip name"
                                        value={newTrip.trip_name}
                                        onChange={(e) => setNewTrip({ ...newTrip, trip_name: e.target.value })}
                                        className="w-full px-3 py-2 mb-3 border border-amber-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                                    />
                                    <input
                                        type="text"
                                    ...
                                    <button
                                        type="submit"
                                        className="w-full bg-gradient-to-r from-amber-600 to-orange-600 text-white py-2 rounded-lg font-semibold hover:shadow-lg transition-all"
                                    >
                                        Create Checklist
                                    </button>
                                </form>
                            ...
                            <div className="space-y-2 max-h-96 overflow-y-auto">
                                {checklists.length === 0 ? (
                                    <p className="text-amber-600 text-sm text-center py-8">No trips yet. Create one!</p>
                                ) : (
                                    checklists.map((checklist) => (
                                        <div
                                            key={checklist.id}
                                            className={`p-3 rounded-lg cursor-pointer transition-all border ${activeChecklist?.id === checklist.id
                                                    ? 'bg-gradient-to-r from-amber-100 to-orange-100 border-amber-500 shadow-md'
                                        ...
                                        >
                                            ...
                                            >
                                                <p className="font-semibold text-amber-900">{checklist.trip_name}</p>
                                                <p className="text-xs text-amber-700">{checklist.destination}</p>
                                                <p className="text-xs text-amber-600 mt-1">{new Date(checklist.travel_date).toLocaleDateString()}</p>
                                            </button>
                                            <button
                    ...
                    <div className="lg:col-span-3">
                        {activeChecklist ? (
                            <>
                                {/* Progress Bar */}
                                <div className="bg-white rounded-2xl shadow-lg p-6 border border-amber-200/50 mb-6">
                                    <div className="flex items-center justify-between mb-4">
                                        <div>
                                            <h3 className="text-2xl font-bold text-amber-900">{activeChecklist.trip_name}</h3>
                                            <p className="text-amber-700">📍 {activeChecklist.destination} • ✈️ {new Date(activeChecklist.travel_date).toLocaleDateString()}</p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-4 mb-4">
                                        <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-lg border border-amber-200">
                                            <p className="text-sm text-amber-700 font-semibold">Overall Progress</p>
                                ...
                                <div className="space-y-6">
                                    {activeChecklist.items.map((category, catIndex) => (
                                        <div key={catIndex} className="bg-white rounded-2xl shadow-lg p-6 border border-amber-200/50">
                                                ...
                                                {category.items.map((item, itemIndex) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex-1">
                                                            <p className={`font-medium ${item.completed ? 'line-through text-gray-500' : 'text-amber-900'}`}>
                                                                {item.name}
                                                            </p>
                            ...
                            <div className="bg-white rounded-2xl shadow-lg p-12 border border-amber-200/50 text-center">
                                <div className="text-6xl mb-4">📋</div>
                                <h3 className="text-2xl font-bold text-amber-900 mb-2">No Checklist Selected</h3>
                                <p className="text-amber-700 mb-6">Create a new trip checklist to get started!</p>
                                <button
